...
spot_template =         '        <Spot ID="{id}" name="{name} SPOT_{id}" VISIBILITY="1" RADIUS="10.0" QUALITY="-1.0" SOURCE_ID="0" POSITION_T="{frame}.0" POSITION_X="{x}" POSITION_Y="{y}" FRAME="{frame}" POSITION_Z="{z}" TISSUE_TYPE="{t_id}" />\n'
...
edge_template =             '        <Edge SPOT_SOURCE_ID="{source_id}" SPOT_TARGET_ID="{target_id}" LINK_COST="-1.0" VELOCITY="{velocity}" DISPLACEMENT="{displacement}" TISSUE_TYPE="{t_id}" TIME="{time}" />\n'
    ...
    <SetupAssignments>
      ...
      <MinMaxGroups>
        <MinMaxGroup>
          ...
          <currentMin>0.0</currentMin>
          <currentMax>65535.0</currentMax>
        </MinMaxGroup>
      </MinMaxGroups>
    </SetupAssignments>
    <Bookmarks>
      <Bookmark>
        <key>1</key>
        <transform>54.88200641601564 0.0 0.0 -148.30700938875373 0.0 54.88200641601564 0.0 -38.50131809285324 0.0 0.0 54.88200641601564 0.7109230078125041</transform>
      </Bookmark>
    </Bookmarks>
  </GUIState>  
</TrackMate>'''
